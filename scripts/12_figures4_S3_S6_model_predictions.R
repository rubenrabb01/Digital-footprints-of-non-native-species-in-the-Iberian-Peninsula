...
if (exists("ANALYSIS_MODE") && ANALYSIS_MODE == "COMMENTS_ONLY") {
  for (ext in c(".png", ".tiff")) {
    src <- file.path(FIG_SENS_DIR, paste0("Figure4_panel_Figure_4_ONECOL_TWOROWS_separate_legends_COMMENTS_ONLY", ext))
    dst <- file.path(FIG_SUPP_DIR, paste0("Figure_S6_model_sensitivity", ext))
    if (file.exists(src)) file.rename(src, dst)
  }
  # keep only selected sensitivity model figure
  keep <- file.path(FIG_SUPP_DIR, c("Figure_S6.png",
                                    "Figure_S6.tiff",
                                    "Figure_S6_model_sensitivity.png",
                                    "Figure_S6_model_sensitivity.tiff"))
  unlink(setdiff(list.files(FIG_SENS_DIR, pattern = "^Figure|^kw_|^Fig", full.names = TRUE), keep), force = TRUE)
} else {
  for (ext in c(".png", ".tiff")) {
    ...
    dsts3 <- file.path(FIG_SUPP_DIR, paste0("Figure_S3", ext))
    if (file.exists(srcs3)) file.copy(srcs3, dsts3, overwrite = TRUE)

    srcs6 <- file.path(FIG_MAIN_DIR, paste0("models_multinom/FigureS6_panel_AB_separate_legends_NO_COMMENTS", ext))
    if (!file.exists(srcs6)) srcs6 <- file.path(FIG_MAIN_DIR, paste0("FigureS6_panel_AB_separate_legends_NO_COMMENTS", ext))
    dsts6 <- file.path(FIG_SUPP_DIR, paste0("Figure_S6", ext))
    if (file.exists(srcs6)) file.copy(srcs6, dsts6, overwrite = TRUE)
  }
