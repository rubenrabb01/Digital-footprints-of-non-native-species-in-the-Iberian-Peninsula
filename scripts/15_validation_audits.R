...
# Supplementary Fig. S7: species-level first-YouTube vs first-record dot plot (all species).
p_lead <- lead_lag %>%
...
FIG_S7_HEIGHT_IN <- max(5, 0.11 * nrow(lead_lag))
ggsave(file.path(FIG_SUPP_DIR, "Figure_S7.png"),
       p_lead, width = 8.0, height = FIG_S7_HEIGHT_IN, dpi = 300, bg = "white")
ggsave(file.path(FIG_SUPP_DIR, "Figure_S7.tiff"),
       p_lead, width = 8.0, height = FIG_S7_HEIGHT_IN, dpi = 300,
       device = "tiff", compression = "lzw", bg = "white")
message("Saved Figure S7 to: ", file.path(FIG_SUPP_DIR, "Figure_S7.png"))

# Supplementary Fig. S8: manual context classification of earliest pre-record videos.
# Change these values if a different exported size is needed.
FIG_S8_WIDTH_MM  <- 230
FIG_S8_HEIGHT_MM <- 120
...
if (!exists("lead_lag_context_summary")) {
  ...
  if (file.exists(context_summary_path)) {
    ...
    message("Using existing context summary to recreate Figure S8: ", context_summary_path)
  }
}
...
if (exists("lead_lag_context_summary") && nrow(lead_lag_context_summary) > 0) {
  ...
  ggsave(file.path(FIG_SUPP_DIR, "Figure_S8.png"),
         p_context, width = FIG_S8_WIDTH_MM, height = FIG_S8_HEIGHT_MM, units = "mm", dpi = 300, bg = "white")
  ggsave(file.path(FIG_SUPP_DIR, "Figure_S8.tiff"),
         p_context, width = FIG_S8_WIDTH_MM, height = FIG_S8_HEIGHT_MM, units = "mm", dpi = 300,
         device = "tiff", compression = "lzw", bg = "white")
  message("Saved Figure S8 to: ", file.path(FIG_SUPP_DIR, "Figure_S8.png"))
} else {
  message("Figure S8 was not created because no completed pre-record context summary was found.")
