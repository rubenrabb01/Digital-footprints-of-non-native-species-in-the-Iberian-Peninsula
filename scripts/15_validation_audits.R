...
read_validation_files <- function(completed_dir, canonical_file, include_patterns, exclude_patterns = character()) {
  ...
  canonical_path <- file.path(canonical_root, canonical_file)

  files <- unlist(lapply(completed_dirs, function(dd) {
  ...
  rows <- purrr::map_dfr(files, function(f) {
    ...
    # Backward compatibility with the original reviewer files.
    if (!"validation_id" %in% names(x) && "review_id" %in% names(x)) x$validation_id <- x$review_id
    if (!"validator2_is_iberian" %in% names(x) && "reviewer2_is_iberian" %in% names(x)) x$validator2_is_iberian <- x$reviewer2_is_iberian
    if (!"validator2_evidence" %in% names(x) && "reviewer2_evidence" %in% names(x)) x$validator2_evidence <- x$reviewer2_evidence
    if (!"validator2_exclusion_reason" %in% names(x) && "reviewer2_exclusion_reason" %in% names(x)) x$validator2_exclusion_reason <- x$reviewer2_exclusion_reason
    if (!"validator2_confidence" %in% names(x) && "reviewer2_confidence" %in% names(x)) x$validator2_confidence <- x$reviewer2_confidence
    if (!"validator2_notes" %in% names(x) && "reviewer2_notes" %in% names(x)) x$validator2_notes <- x$reviewer2_notes
...
if (file.exists(completed_label_path)) {
  ...
  # Older/example consensus files may use reviewer2_* column names.
  # Harmonise them here so the metric code works with both real validator files
  # and placeholder completed files included for testing.
  if (!"validator2_is_iberian" %in% names(label_completed_raw)) {
    if ("reviewer2_is_iberian" %in% names(label_completed_raw)) {
      label_completed_raw$validator2_is_iberian <- label_completed_raw$reviewer2_is_iberian
    } else if (".decision_consensus" %in% names(label_completed_raw)) {
      label_completed_raw$validator2_is_iberian <- label_completed_raw$.decision_consensus
    } else {
      label_completed_raw$validator2_is_iberian <- NA_character_
    }
  }
...
# Shared print-friendly palette for the lead/lag panel figure.
tol_muted <- c(
  "#332288", "#88CCEE", "#44AA99", "#117733",
  "#999933", "#DDCC77", "#CC6677", "#882255"
)
lead_lag_cols <- c(
  "YouTube before official first-record year" = tol_muted[7],
  "YouTube in same year as official first record" = tol_muted[6],
  "YouTube after official first-record year" = tol_muted[1]
)

# New Supplementary Fig. S8: combined lead/lag + pre-record context panel.
# Panel A splits the species-level lead/lag plot into three continuing columns to
# keep species labels readable. Panel B shows the manual context classification
# of earliest pre-record YouTube videos.
lead_lag_plot_df <- lead_lag %>%
...
lead_group_size <- ceiling(nrow(lead_lag_plot_df) / 3)
lead_lag_col1 <- lead_lag_plot_df %>% filter(.data$row_id <= lead_group_size)
lead_lag_col2 <- lead_lag_plot_df %>% filter(.data$row_id > lead_group_size & .data$row_id <= 2 * lead_group_size)
lead_lag_col3 <- lead_lag_plot_df %>% filter(.data$row_id > 2 * lead_group_size)
lead_x_limits <- range(lead_lag_plot_df$lead_lag_years, na.rm = TRUE)
lead_x_pad <- diff(lead_x_limits) * 0.04
lead_x_limits <- c(lead_x_limits[1] - lead_x_pad, lead_x_limits[2] + lead_x_pad)

make_lead_col <- function(dat) {
  dat <- dat %>%
    arrange(.data$lead_lag_years) %>%
    mutate(CleanName_plot = factor(.data$CleanName_plot, levels = rev(.data$CleanName_plot)))

  ggplot(dat, aes(x = lead_lag_years, y = CleanName_plot, colour = year_category)) +
    geom_vline(xintercept = 0, linetype = "dashed", colour = "grey35") +
    geom_point(size = 2.0, alpha = 0.90) +
    scale_colour_manual(values = lead_lag_cols, name = NULL, drop = FALSE) +
    guides(colour = guide_legend(
      nrow = 1,
      byrow = TRUE,
      override.aes = list(size = 3.0)
    )) +
    coord_cartesian(xlim = lead_x_limits) +
    labs(
      x = NULL,
      y = NULL
    ) +
    ...
    theme(
      legend.position = "top",
      legend.text = element_text(size = 10, margin = margin(r = 18)),
      legend.spacing.x = grid::unit(1.8, "cm"),
...
p_lead_col1 <- make_lead_col(lead_lag_col1)
p_lead_col2 <- make_lead_col(lead_lag_col2)
p_lead_col3 <- make_lead_col(lead_lag_col3)
lead_legend <- cowplot::get_legend(p_lead_col1)
p_lead_triplet <- cowplot::plot_grid(
  p_lead_col1 + theme(legend.position = "none"),
  p_lead_col2 + theme(legend.position = "none"),
  p_lead_col3 + theme(legend.position = "none"),
  ncol = 3, rel_widths = c(1, 1, 1), align = "hv",
  labels = c("A)", "", ""), label_size = 13, label_fontface = "plain"
)

# Use one shared x-axis title for panel A. This avoids clipping of the
# long title inside the middle-column viewport while keeping the label
# visually centred below the three lead/lag columns.
p_lead_x_title <- cowplot::ggdraw() +
  cowplot::draw_label(
    "First YouTube video relative to official first record (years)",
    x = 0.5, y = 0.55, hjust = 0.5, vjust = 0.5,
    size = 12
  )

p_lead_panel <- cowplot::plot_grid(
  lead_legend,
  p_lead_triplet,
  p_lead_x_title,
  ncol = 1,
  rel_heights = c(0.09, 1, 0.075)
)

# Manual context classification of earliest pre-record videos.
if (!exists("lead_lag_context_summary")) {
  ...
  if (file.exists(context_summary_path)) {
    ...
    message("Using existing context summary to recreate panel B in Figure S8: ", context_summary_path)
  }
}

if (exists("lead_lag_context_summary") && nrow(lead_lag_context_summary) > 0) {
  context_cols <- c(
    "Trade / pet / aquarium / ornamental" = tol_muted[1],
    "Plausible Iberian detection / observation" = tol_muted[3],
    "Media / news / institutional communication" = tol_muted[2],
    "General species information" = tol_muted[6],
    "Non-occurrence / unclear Iberian relevance" = tol_muted[7],
    "Uncertain" = "#777777"
  )
  p_context <- lead_lag_context_summary %>%
    ...
    geom_col(width = 0.7, colour = "grey25", linewidth = 0.20) +
    ...
    coord_cartesian(xlim = c(0, max(lead_lag_context_summary$n_species, na.rm = TRUE) * 1.27), clip = "off") +
    theme_light(base_size = 11) +
    theme(
      axis.text.x = element_text(size = 10),
      ...
      panel.grid.minor = element_blank(),
      panel.grid.major.y = element_blank(),
      plot.margin = margin(8, 30, 8, 90)
    )

  # Put panel B below panel A, but make it occupy approximately two of the
  # three upper columns. The blank side panels keep the lower plot from
  # stretching too widely while the larger left margin prevents y-axis labels
  # from being clipped.
  blank_panel <- ggplot() + theme_void()
  p_context_row <- cowplot::plot_grid(
    blank_panel,
    p_context,
    blank_panel,
    ncol = 3,
    rel_widths = c(0.00, 2.0, 0.95),
    labels = c("", "B)", ""),
    label_size = 13,
    label_fontface = "plain"
  )

  Figure_S8_panel <- cowplot::plot_grid(
    p_lead_panel,
    p_context_row,
    ncol = 1,
    rel_heights = c(2.05, 1.22),
    labels = c("", ""), label_size = 13, label_fontface = "plain"
  )

  fig_s8_width <- 12.0
  # Slightly taller export gives panel B more vertical space without widening
  # the full figure or stretching the lower plot horizontally.
  fig_s8_height <- max(9.1, 0.05 * nrow(lead_lag_plot_df) + 3.0)

  ggsave(file.path(FIG_SUPP_DIR, "Figure_S8.png"),
         Figure_S8_panel, width = fig_s8_width, height = fig_s8_height,
         dpi = 300, bg = "white")
