...
#
# Adapted from the submitted Figure S4 + S5 script.
...
# and export dimensions used in the old Figure S4/S5 script.
# ============================================================
...
# ============================================================
ggsave(file.path(FIG_SUPP_DIR, "Figure_S4.png"),
       Fig_S4_panel_core, width = 375, height = 375, units = "mm", dpi = 300, bg = "white")
ggsave(file.path(FIG_SUPP_DIR, "Figure_S4.tiff"),
       Fig_S4_panel_core, width = 375, height = 375, units = "mm", dpi = 300, bg = "white")
...
readr::write_csv(pred_marg,  file.path(TAB_SUPP_DIR, "Figure_S4_weighted_predicted_probabilities.csv"))
readr::write_csv(pred_delta, file.path(TAB_SUPP_DIR, "Figure_S4_weighted_post_pre_deltas.csv"))
readr::write_csv(pred_bar,   file.path(TAB_SUPP_DIR, "Figure_S4_weighted_barplot_ci_data.csv"))
readr::write_csv(cal,        file.path(TAB_SUPP_DIR, "Figure_S5_calibration_observed_vs_predicted.csv"))

aic_summary <- tibble::tibble(
  model = c("unweighted_step_model", "view_weighted_step_model"),
  AIC = c(AIC(m_step), AIC(m_w_step))
)
readr::write_csv(aic_summary, file.path(TAB_SUPP_DIR, "Figure_S4_S5_model_AIC_summary.csv"))

cat("\nUNWEIGHTED AIC:", AIC(m_step), "\n")
cat("WEIGHTED   AIC:", AIC(m_w_step), "\n")
message("Figures S4 and S5 exported to: ", FIG_SUPP_DIR)
message("Figure S4/S5 source data exported to: ", TAB_SUPP_DIR)
