...
The manuscript analyses are reproduced from the curated files in `data/paper_input/` and the supporting lineage files in `data/preparation_input/`. The example script demonstrates the basic structure of a single API query using one example search term and a placeholder API key.

The example does not reproduce the full search strategy, batching, filtering, validation, or corpus construction workflow used in the study. Those steps are described in the manuscript, Supplementary Methods, and repository documentation.
