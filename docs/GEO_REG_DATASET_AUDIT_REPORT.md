...
The GEO retrieval file contains **305 unique videos** from the geotag/location-based retrieval pathway. This set was identified consistently from intermediate working files containing a `source_flag` column by selecting records marked as `GEO` or `BOTH`.
...
| GEO retrieval file | `data/preparation_input/api_geotagged_anchor_sourceflag.csv` (**GEO retrieval file**) | 305 | 305 | Geotag/location-based GEO records identified from intermediate files containing a `source_flag` column. |
...
- `docs/source_flag_file_summary.csv`: counts of `source_flag` values in the uploaded working files.
