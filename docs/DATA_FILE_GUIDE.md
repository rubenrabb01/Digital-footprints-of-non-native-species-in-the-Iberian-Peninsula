...
**Rows:** 1,895 distinct video-level records  
...
Anonymised validator-specific validation templates and completed validation inputs are stored under `data/validation_input/`. Completed validator files should be placed under `data/validation_input/completed/validation_files_by_validator/` before rerunning `scripts/99_run_validation_audits.R`. The completed pre-record context file used for Figure S8 is stored under `data/validation_input/completed/lead_lag_context/`. The validation runner writes summary tables to `outputs/validation/tables/`, script-generated consensus files to `outputs/validation/completed/`, and paper-ready validation panels to `outputs/figures/supplement/` (`Figure_S8` and `Figure_S9`).
