...
The repository is organised into the following main parts:

```text
data/                         Curated input data and supporting lineage files
data_preparation_workflow/    Scripts documenting the upstream data-preparation pathway
...
This reads the completed validation files and writes validation summaries and supplementary validation figures.
...
The outputs are written to `outputs/figures/exploratory/` and can be inspected separately from the manuscript figures.
...
Validation files are stored under `data/validation_input/`, with compatibility copies under `outputs/validation/`. The repository includes validation templates, completed validation files, decision rules, summary tables, and the combined validation figure used in the revised manuscript and Supplementary Material.
